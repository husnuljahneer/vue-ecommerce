...
<section v-if="this.userProducts.length  > 1">
  ...
  <div class="flex flex-col items-center justify-between pb-6 mt-40 h-window">
    <h1 class=" text-lg sm:text-lg md:text-3xl lg:text-6xl ">
      ORDER HISTORY
    </h1>
    ...
    >
      ...
      <div class="w-full md:w-2/3 flex flex-col mb-16 items-center text-center">
        ...
        <p class="mb-8 leading-relaxed">
          You haven't ordered anything yet. You can order your favourite items from the shop. 
        </p>
      </div>
    </div>
  </section>
...
export default {
  ...
  methods: {
    getUserProducts(userId) {
      CartService.getUserProducts(userId).then((response) => {
        // alert(response.data.product.length);
        // alert(response.data.product[0].order_details.length);
      
        for (let i = 0; i < response.data.product.length; i++) {
          for (let j = 0; j < response.data.product[i].order_details.length; j++) {
  ...
  created() {
    ...
    // // alert("You are logged in" + this.userId);
  },
};
</script>